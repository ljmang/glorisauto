 ...
 * 使用 parentID：null 为一级，parentID === 父级.id 为二级
...
/**
 * 将扁平分类列表转为树：只保留两级，roots 为一级（parentID == null），其 children 为二级（parentID === root.id）
 */
export function buildCategoryTree(categories: CategoryWithRelation[]): CategoryTreeNode[] {
  ...
  for (const c of rootItems) {
    const parentId = c.id;
    const children = list
      .filter((item) => item.parentID != null && item.parentID === parentId)
      .sort((a, b) => getSort(a) - getSort(b))
      .map((item) => ({
        id: item.id,
